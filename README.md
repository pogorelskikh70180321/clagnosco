...
Данная программа имеет 4 варианта запуска:
...
3.	«main.py» – Python-файл, работающий на персональном компьютере.
...
3.	Запустите весь код из пункта «Импорт из GitHub <span>https://github.com/pogorzelskich/clagnosco.git</span>». Загрузка обычно занимает примерно 1-2 минуты.
...
Если имеется сохранённая CSV-таблица от предыдущей обработки, то её можно импортировать как сохранение сеанса.
...
## 6. Перемещение
Чтобы переместить проект, нужно удостовериться, что он закрыт.

Дополнительное условия для перемещения main.ipynb не требуются.

Для перемещения Clagnosco.exe вместе с файлом нужно переместить и папку «models».

Для main.py и Clagnosco_start.bat нужно переместить всю папку «clagnosco», в которой они находятся.

## 7. Удаление
Удаление программы является чистым (для всех вариантов кроме main.py и Clagnosco_start.bat) и осуществляется простой отправкой в корзину.
...
При main.py и Clagnosco_start.bat нужно удалить всю папку «clagnosco», в которой они находятся. Также стоит отметить, что установленные модули из requirements.txt не удаляются, так как могут удалиться библиотеки, нужные пользователю.

Также можно удалить ярлыки на рабочем столе, созданные для этой программы.

### 7.1. Очистка кэша
Очистка кэша изображений может выполняться в самой программе, удаление файлов кэша его можно осуществить, удалив все файлы, заканчивающиеся на «_latent.npy» из папок, в которых сохранился кэш.
